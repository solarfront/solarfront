...
export function startWorker() {
  ...
  app.post(
    ...
    gatekeeper.httpHandler(LimiterType.Post, async (req, res) => {
      ...
      const game = await gm.createGame(id, gc);
  ...
  app.post(
    ...
    gatekeeper.httpHandler(LimiterType.Post, async (req, res) => {
      ...
      log.info(
        `[Worker] Received /api/start_game request for gameID: ${gameId}`,
      );

      const game = gm.game(gameId);
      if (!game) {
        log.error(
          `[Worker] Game not found for /api/start_game. gameID: ${gameId}`,
        );
        return res.status(404).json({ error: "Game not found to start" });
      }
      if (game.isPublic()) {
        const clientIP = req.ip || req.socket.remoteAddress || "unknown";
        log.warn(
          `[Worker] Cannot start public game via API. gameID: ${game.id}, ip: ${ipAnonymize(clientIP)}`,
        );
        return res
          .status(400)
          .json({ error: "Public games start automatically" });
      }

      log.info(
        "[Worker] Before calling game.start(), current game.activeClients:",
        {
          gameID: gameId,
          count: game.activeClients.length,
          clients: game.activeClients.map((c) => ({
            clientID: c.clientID,
            username: c.username,
            playerID: c.playerID,
          })),
      ...
      gatekeeper.wsHandler(req, async (message: string) => {
        ...
        try {
          ...
          if (clientMsg.type === "join") {
            //log.info(`[dbg] join message: ${JSON.stringify(clientMsg)}`);
  ...
  app.use((err: Error, req: Request, res: Response, next: NextFunction) => {
    log.error(
      `[Worker] Unhandled error in ${req.method} ${req.originalUrl}:`,
      err,
    );
    // Ensure a JSON response for all errors if headers haven't been sent
    if (!res.headersSent) {
      res.status(500).json({
        error: "An unexpected server error occurred",
        details: err.message,
        stack: config.env() === GameEnv.Dev ? err.stack : undefined, // Only send stack in dev
      });
    } else {
