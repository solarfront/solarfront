...
export class TransformHandler {
  public scale: number = 0.37;
  private offsetX: number = 750;
  private offsetY: number = 1700;
  ...
  onGoToPlayer(event: GoToPlayerEvent) {
    ...
    // Check if player has spawned and has a name location
    const nameLocation = event.player.nameLocation();
    if (!nameLocation) {
      console.warn(`Player ${event.player.displayName()} has not spawned yet`);
      return;
    }
