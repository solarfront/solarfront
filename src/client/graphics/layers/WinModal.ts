...
export class WinModal extends LitElement implements Layer {
  ...
  tick() {
    ...
    if (
      ...
      this.soundManager.stopBackgroundMusic(false); // Stop music instantly
      // Play defeat sound after 100ms delay
      setTimeout(() => {
        this.soundManager.playDefeatJingle();
      }, 100);
      this.show();
    }
    ...
    winUpdates.forEach((wu) => {
      if (wu.winnerType === "team") {
        ...
        if (wu.winner === this.game.myPlayer()?.team()) {
          ...
          this.soundManager.stopBackgroundMusic(false); // Stop music instantly
          // Play victory sound after 100ms delay
          setTimeout(() => {
            this.soundManager.playVictory1stPlace();
          }, 100);
        } else {
