...
export class RadialMenu implements Layer {
  ...
  private createMenuElement() {
    ...
    const pie = d3
      ...
      .value(() => 1)
      .padAngle(0.03)
      .startAngle((19 * Math.PI) / 36) // Start at 95 degrees (65 + 30 degrees clockwise)
      .endAngle(2 * Math.PI + (19 * Math.PI) / 36); // Complete the circle but shifted by 95 degrees
  ...
  async tick() {
    ...
    // Add validation to prevent invalid coordinates
  ...
  private onContextMenu(event: ContextMenuEvent) {
    ...
    // Check if clickedCell is null or coordinates are invalid
    if (
      this.clickedCell === null ||
      !this.g.isValidCoord(this.clickedCell.x, this.clickedCell.y)
    ) {
      this.hideRadialMenu();
      return;
    }
  ...
  private handlePlayerActions(
    ...
    if (this.g.hasOwner(tile)) {
      ...
      // Add whisper option for other players' territories
      const tileOwner = this.g.owner(tile);
      if (tileOwner.isPlayer() && tileOwner !== myPlayer) {
        this.activateMenuElement(Slot.Whisper, "#9333ea", chatIcon, () => {
          // Open whisper tab with this player
          const chatDisplay = document.querySelector("chat-display") as any;
          if (chatDisplay) {
            chatDisplay.openWhisperTab(tileOwner.id(), tileOwner.name());
          }
        });
      }
    }
    ...
    if (
      ...
      this.activateMenuElement(Slot.Spaceship, "#3f6ab1", spaceshipIcon, () => {
