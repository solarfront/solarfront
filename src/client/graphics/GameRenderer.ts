...
import { HotkeyBar } from "./layers/HotkeyBar";
import { Layer } from "./layers/Layer";
import { Leaderboard } from "./layers/Leaderboard";
import { MusicController } from "./layers/MusicController";
import { MultiSelectButton } from "./layers/MultiSelectButton";
...
import { SpawnGuidancePopup } from "./layers/SpawnGuidancePopup";
import { SpawnTimer } from "./layers/SpawnTimer";
import { TutorialPopup } from "./layers/TutorialPopup";
...
export function createRenderer(
  ...
  controlPanel.transformHandler = transformHandler;

  const hotkeyBar = document.querySelector("hotkey-bar") as HotkeyBar;
  if (hotkeyBar && hotkeyBar instanceof HotkeyBar) {
    hotkeyBar.clientID = clientID;
    hotkeyBar.eventBus = eventBus;
    hotkeyBar.game = game;
    hotkeyBar.transformHandler = transformHandler;
    hotkeyBar.init();
  }

  const multiSelectButton = document.querySelector("multi-select-button") as MultiSelectButton;
  if (multiSelectButton && multiSelectButton instanceof MultiSelectButton) {
    multiSelectButton.eventBus = eventBus;
    multiSelectButton.init();
  }
  ...
  const layers: Layer[] = [
    ...
    new MusicController(game), // Add music controller to handle background music
    eventsDisplay,
    chatDisplay,
    buildMenu,
    new RadialMenu(
    ...
    new SpawnGuidancePopup(game, clientID),
    ...
    playerPanel,
    multiTabModal,
    new TutorialPopup(game, clientID), // Render last so it appears on top
  ];
