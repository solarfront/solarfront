...
export class UserSettingModal extends LitElement {
  ...
  private toggleTutorials(e: CustomEvent<{ checked: boolean }>) {
    const enabled = e.detail?.checked;
    if (typeof enabled !== "boolean") return;

    localStorage.setItem("tutorialsEnabled", enabled.toString());
    console.log("🎓 Tutorial Messages:", enabled ? "ON" : "OFF");

    this.requestUpdate();
  }

  private toggleAutoPlayOnSpawn(e: CustomEvent<{ checked: boolean }>) {
    const enabled = e.detail?.checked;
    if (typeof enabled !== "boolean") return;

    localStorage.setItem("settings.autoPlayOnSpawn", enabled.toString());
    console.log("🤖 Auto Play on Spawn:", enabled ? "ON" : "OFF");
  ...
  private renderBasicSettings() {
    return html`
      ...
      <!-- 🎓 Tutorial Messages -->
      <setting-toggle
        label="Tutorial Messages"
        description="Show helpful tutorial messages during gameplay"
        id="tutorial-toggle"
        .checked=${localStorage.getItem("tutorialsEnabled") !== "false"}
        @change=${this.toggleTutorials}
      ></setting-toggle>

      <!-- 🤖 Auto Play on Spawn -->
      <setting-toggle
        label="Auto Play on Spawn"
        description="Enable AI assistance automatically when spawning into the game"
        id="auto-play-spawn-toggle"
        .checked=${localStorage.getItem("settings.autoPlayOnSpawn") !== "false"}
        @change=${this.toggleAutoPlayOnSpawn}
      ></setting-toggle>
