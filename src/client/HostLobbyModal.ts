...
export class HostLobbyModal extends LitElement {
  ...
  @state() private selectedMap: GameMapType = GameMapType.Space1;
              ...
              <label for="bots-count" class="option-card">
                ...
                >
                  ${[
                    ...
                    [UnitType.Viper, "unit_type.viper"],
                    [UnitType.Condor, "unit_type.condor"],
                    ...
                    // [UnitType.MIRV, "unit_type.mirv"], // World-Breakers disabled
                  ].map(
  ...
  private async putGameConfig() {
    ...
    const response = await fetch(
      `${process.env.API_URL}/${config.workerPath(this.lobbyId)}/api/game/${this.lobbyId}`,
      {
        method: "PUT",
        headers: {
  ...
  private async startGame() {
    ...
    const config = await getServerConfigFromClient();
    const response = await fetch(
      `${process.env.API_URL}/${config.workerPath(this.lobbyId)}/api/start_game/${this.lobbyId}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function createLobby(): Promise<GameInfo> {
  ...
  try {
    const id = generateID();
    const response = await fetch(
      `${process.env.API_URL}/${config.workerPath(id)}/api/create_game/${id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
