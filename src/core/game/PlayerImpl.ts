...
export class PlayerImpl implements Player {
  ...
  // Port selection for round-robin spawning
  private _nextPortIndex = 0;

  // Construction rate limiting to prevent mashing exploits
  private _lastConstructionTick = new Map<UnitType, number>();
  private _lastAnyConstructionTick = 0; // Track last construction tick of ANY building type
  private readonly CONSTRUCTION_COOLDOWN_TICKS = 2; // 2 ticks (200ms at 10 ticks/second) between same building type
  private readonly GLOBAL_CONSTRUCTION_COOLDOWN_TICKS = 2; // 2 ticks (200ms at 10 ticks/second) between ANY building
  ...
  constructor(
    ...
    // Normalize nation flags for FakeHuman players to approved set.
    if (this.playerInfo.playerType === PlayerType.FakeHuman) {
      const allowed = [
        "nation1",
        "nation2",
        "nation4",
      ...
      if (!this._flag || !allowed.includes(this._flag)) {
        const idx = simpleHash(this.playerInfo.id) % allowed.length;
        this._flag = allowed[idx];
      }
    }
  ...
  toUpdate(): PlayerUpdate {
    ...
    return {
      ...
      ticksAliveFor: this.ticksAliveFor(),
    };
  }
  ...
  ticksAliveFor(): number {
    return this._ticksAliveFor;
  }
  updateTicksAliveFor(): void {
    if (this.isAlive()) {
      this._ticksAliveFor++;
    }
  }
  ...
  public buildableUnits(tile: TileRef): BuildableUnit[] {
    ...
    return Object.values(UnitType).map((u) => {
      return {
        type: u,
        canBuild:
          this.mg.inSpawnPhase() && !this.mg.config().instantBuild()
            ? false
  ...
  canBuild(
    ...
    switch (unitType) {
      ...
      case UnitType.MIRV:
        return false; // MIRVs are disabled in this build
      // Original MIRV code:
  ...
  nukeSpawn(tile: TileRef): TileRef | false {
    ...
    // Allow nuking teammates in team mode - removed team check
    // if (owner.isPlayer()) {
  ...
  warshipSpawn(tile: TileRef): TileRef | false {
    // Allow warships to spawn on any tile (amphibious capability)
    // Removed ocean-only restriction

    // Use the closest port to the requested tile instead of round-robin
    const selectedPort = this.getClosestAvailablePortForWarship(tile);
    if (!selectedPort) {
      consolex.log(`No available ports for warship construction for player ${this.name()}`);
      return false;
    }

    // Double-check the port is still available (prevent race conditions from rapid hotkey pressing)
    if (selectedPort.hasPortQueue()) {
      consolex.log(`Port at ${selectedPort.tile()} already has a queue, rejecting warship spawn`);
      return false;
    }

    return selectedPort.tile();
  }

  // Get next available port for warship construction using round-robin
  getNextAvailablePortForWarship(): Unit | null {
    const allPorts = this.units(UnitType.Port);
    if (allPorts.length === 0) {
      return null;
    }

    // Find next available port using round-robin selection
    const availablePorts = allPorts.filter(port => !port.hasPortQueue());
    if (availablePorts.length === 0) {
      return null; // All ports are busy
    }

    // Use round-robin selection among available ports
    const portIndex = this._nextPortIndex % availablePorts.length;
    const selectedPort = availablePorts[portIndex];

    // Update index for next call
    this._nextPortIndex = (this._nextPortIndex + 1) % availablePorts.length;

    return selectedPort;
  }

  // Get the closest available port to target tile for warship construction
  getClosestAvailablePortForWarship(targetTile: TileRef): Unit | null {
    const allPorts = this.units(UnitType.Port);
    if (allPorts.length === 0) {
      return null;
    }

    // Find available ports (not currently building anything)
    const availablePorts = allPorts.filter(port => !port.hasPortQueue());
    if (availablePorts.length === 0) {
      return null; // All ports are busy
    }

    // Find the closest port to the target tile
    let closestPort = availablePorts[0];
    let minDistance = this.mg.manhattanDist(closestPort.tile(), targetTile);

    for (let i = 1; i < availablePorts.length; i++) {
      const port = availablePorts[i];
      const distance = this.mg.manhattanDist(port.tile(), targetTile);
  ...
  // Construction rate limiting methods
  getLastConstructionTick(unitType: UnitType): number | undefined {
    return this._lastConstructionTick.get(unitType);
  }

  setLastConstructionTime(unitType: UnitType): void {
    const currentTick = this.mg.ticks();
    this._lastConstructionTick.set(unitType, currentTick);
    this._lastAnyConstructionTick = currentTick; // Also update global construction tick
  }

  canConstructBuilding(unitType: UnitType): boolean {
    const currentTick = this.mg.ticks();

    // Check global cooldown first (applies to ANY building)
    if ((currentTick - this._lastAnyConstructionTick) < this.GLOBAL_CONSTRUCTION_COOLDOWN_TICKS) {
      return false;
    }

    // Then check specific building type cooldown
    const lastTick = this._lastConstructionTick.get(unitType);
    if (!lastTick) return true;
    return (currentTick - lastTick) >= this.CONSTRUCTION_COOLDOWN_TICKS;
  }
