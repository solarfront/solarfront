...
function getRandomPortrait(random: PseudoRandom): string {
  // Allowed portraits: nation1, nation2, nation4, nation5, nation6, nation7
  const allowed = [1, 2, 4, 5, 6, 7];
  const idx = random.nextInt(0, allowed.length - 1);
  return `nation${allowed[idx]}`;
}
...
export async function createGameRunner(
  ...
  // Only use nations from the map, no fake players
  const allAI = nations;
...
export class GameRunner {
  ...
  init() {
    ...
    // Add nation executions (nations controlled by FakeHumanExecution AI)
