...
export class WorkerClient {
  ...
  initialize(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        "WorkerClient: Starting initialization with messageId:",
        messageId,
      );

      this.messageHandlers.set(messageId, (message) => {
        if (message.type === "initialized") {
          console.log("WorkerClient: Received initialized message");
          this.isInitialized = true;
          resolve();
        }
      });

      console.log("WorkerClient: Sending init message to worker");
      ...
      setTimeout(() => {
        if (!this.isInitialized) {
          console.error(
            "WorkerClient: Initialization timeout after 15 seconds",
          );
