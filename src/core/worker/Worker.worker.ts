...
ctx.addEventListener("message", async (e: MessageEvent<MainThreadMessage>) => {
  ...
  switch (message.type) {
    ...
    case "init":
      try {
        console.log("WebWorker: Starting initialization...");
        gameRunner = createGameRunner(
          ...
          .then((gr) => {
            console.log(
              "WebWorker: Initialization successful, sending initialized message",
            );
            sendMessage({
              type: "initialized",
              id: message.id,
          ...
          .catch((error) => {
            console.error(
              "WebWorker: Failed to initialize game runner:",
              error,
            );
            // Still send the initialized message even if there's an error
            // so the main thread doesn't timeout
            sendMessage({
              type: "initialized",
              id: message.id,
            } as InitializedMessage);
        ...
        console.error("WebWorker: Failed to start initialization:", error);
        // Send initialized message to prevent timeout
        sendMessage({
          type: "initialized",
          id: message.id,
        } as InitializedMessage);
