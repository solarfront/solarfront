...
export class SAMLauncherExecution implements Execution {
  ...
  private isHit(type: UnitType, random: number): boolean {
    // 100% interception rate for all nuke types
    return true;
  }
  ...
  tick(ticks: number): void {
    ...
    // HP regeneration: 1 HP per tick (same as Orbital Cannon)
    const maxHealth = this.sam.info().maxHealth;
    if (maxHealth && this.sam.health() < maxHealth) {
      this.sam.modifyHealth(1);
    }
  }
